...
- `fund_agent/service` 负责 `import_local_report`、`read_local_report`、`list_reports`、`extract_fee_rates`、`extract_performance_returns` use case，编排本地 PDF 导入、completed catalog 复用、必要时 Docling conversion fallback、Service-level routing attempts audit、disclosure locator contract registry、`fee_rates` 多目标阅读定位、`performance_returns` 业绩表现披露定位、三项当前适用年费率受控抽取、`past_1_year` 净值增长率/业绩比较基准收益率受控抽取和 Host 调用。
...
当前不是自动报告、投资判断或发布就绪系统；`performance_returns` 默认 reading locator 只定位披露和 citation；`extract_performance_returns` 只在 Service 层基于已定位表格抽取 `past_1_year` 的 `nav_growth_rate` / `benchmark_return_rate` DTO，不计算差额或年化收益；`extract_fee_rates` 只覆盖已裁决的三项费率字段，不扩展为通用字段抽取框架。
