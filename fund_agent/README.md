...
- `fund_agent/service` 负责 `import_local_report`、`read_local_report`、`list_reports` 三个 use case，编排本地 PDF 导入、completed catalog 复用、必要时 Docling conversion fallback、三类 hardcoded controlled query profile routing、Service-level routing attempts audit 和 Host 调用。
