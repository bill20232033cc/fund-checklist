...
LLM_ANALYSIS_PROMPTS: dict[int, str] = {
    ...
    6: (
        ...
        "【逐步分析框架】\n"
        "第一步：识别核心风险\n"
        "- 从数据表中找出最关键的 1-2 个风险，区分结构性风险（长期存在）和阶段性风险（短期波动）\n"
        "- 必须明确指出哪一个是最关键的风险（格式：「最关键的风险是 [名称]」）\n"
        "- 必须使用「结构性风险」和「阶段性风险」这两个术语来分类风险\n"
        "- 禁止只并列列出风险而不排序\n"
        "- 引用具体数据说明为什么这是核心风险\n"
        "\n"
        "第二步：风险严重程度分级\n"
        "- 分为三级：高风险（数据缺失/异常导致无法评估）、中风险（有数据但趋势恶化）、低风险（数据正常无异常信号）\n"
        "- 每级必须引用数据表中的具体数字作为依据\n"
        "- 禁止使用「一票否决」「致命」「不可接受」等绝对化表述\n"
        "- 【分级与语言强度校准 — 必须严格遵守】\n"
        "  - 风险分级为「低」时：使用克制语言（如「需关注」「存在可能」「若…则…」），禁止「双杀」「崩盘」「暴跌」「危机」「显著回撤」等灾难性词汇\n"
        "  - 风险分级为「中」时：可指出恶化趋势，但需保留条件句（如「若趋势持续…可能导致…」），不得断言后果\n"
        "  - 风险分级为「高」时：可直述严重性和紧迫性，但仍禁止「一票否决」「致命」「不可接受」\n"
        "  - 通则：分级标签和描述语言必须自洽——「低风险」不应读到暗示极端后果的描述\n"
        "\n"
        "第三步：信息缺口分析（必须回答）\n"
        "- 必须以「当前最大的信息缺口是 [具体字段/数据]」开头，明确写出\n"
        "- 如果这个缺口被填补（获取到正确数据），会如何改变风险判断？\n"
        "- 如果数据确认是错误的，结论会如何变化？如果数据确认是正确的，结论会如何变化？\n"
        "- 【信息缺口表述边界】禁止使用「建议投资者关注/留意/注意/跟踪…」「投资者应/需要/可以…」等句式\n"
        "- 替代写法：「[具体数据] 是补充该信息缺口的关键来源」或「若 [具体数据] 可获取，则 [结论变化]」\n"
        "\n"
...
LLM_PASSIVE_CHAPTER_PROMPTS: dict[int, str] = {
    ...
    6: (
        ...
        "\n"
        "【覆盖范围 — 被动基金三类特有风险】\n"
        "1. 清盘风险：规模过小可能导致清盘，评估基金规模是否在安全区间\n"
        "2. 跟踪偏离长期趋势：分析跟踪误差是否有持续扩大趋势，是否影响投资目标\n"
        "3. 指数风险：跟踪指数本身的结构性风险（行业集中度、估值水平、成分股质量）\n"
        "\n"
        "【逐步分析框架】\n"
        "第一步：识别核心风险\n"
        "- 从上述三类风险中，指出哪一个是当前最关键的风险\n"
        "- 格式：「三类风险中，最关键的风险是 [名称]，原因：[一句话依据]」\n"
        "- 必须使用「结构性风险」和「阶段性风险」这两个术语来分类风险（结构性风险：与基金架构/指数设计相关；阶段性风险：与市场环境/短期波动相关）\n"
        "- 禁止三类风险各写一段而不做优先级排序\n"
        "\n"
        "第二步：风险严重程度分级\n"
        "- 分为三级：高风险（数据缺失/异常）、中风险（趋势恶化）、低风险（数据正常）\n"
        "- 每级必须引用数据表中的具体数字作为依据\n"
        "- 禁止使用「一票否决」「致命」「不可接受」等绝对化表述\n"
        "- 【分级与语言强度校准 — 必须严格遵守】\n"
        "  - 风险分级为「低」时：使用克制语言（如「需关注」「存在可能」），禁止「双杀」「暴跌」「危机」「显著回撤」等灾难性词汇\n"
        "  - 风险分级为「中」时：可指出恶化趋势，但需保留条件句，不得断言后果\n"
        "  - 风险分级为「高」时：可直述严重性，但仍禁止「一票否决」「致命」\n"
        "  - 跟踪偏离：若无持续扩大趋势且超额收益为正值，应归为低风险，风险描述限于「需关注跟踪误差变化」，不得夸大\n"
        "  - 通则：分级标签和描述语言必须自洽\n"
        "\n"
        "第三步：信息缺口分析（必须回答）\n"
        "- 必须以「当前最大的信息缺口是 [具体字段]」开头\n"
        "- 分别说明数据错误/正确时结论如何变化\n"
        "- 禁止使用「建议投资者关注/留意/注意/跟踪」「投资者应/需要」等句式\n"
        "- 替代写法：「[具体数据] 是补充该信息缺口的关键来源」或「若 [具体数据] 可获取，则 [结论变化]」\n"
        "\n"
        "- 包含标准风险声明（过往业绩不代表未来表现）\n"
        "- 可以引用数据表中的数字，但不得编造数据表中不存在的数字"
