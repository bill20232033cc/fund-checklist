...
MVP plan 已关闭。当前已完成到 Post-MVP Slice 9D；Slice 9E 已裁决为 Service routing attempts audit。
...
```text
Service result 是否能用最小 `routing_trace` 记录 routing attempts，使原始 query 直接成功、fallback candidate 成功、全部 candidate `not_found`、非受控 query 四类路径均可审计；不得存 `selected_query`、`selected_index`、score、confidence 或解释字段。CLI 默认输出格式、citation、answer、failure code、Agent `tool_trace`、`search_document` contract、Agent policy、Store search、开放语义理解、embedding、LLM intent、template contract execution、calculation framework、字段抽取、自动报告或投资判断不得混入 Slice 9E。
```
