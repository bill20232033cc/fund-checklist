...
更新时间：2026-08-23（投资者偏好分析 §6.26 新增 §6.26.12 行为证据对照（Slice P5 设计初稿）：确定性规则对照 memos + thought_records 行为文本与问卷声明，持仓变动对照拆 P5b；2026-08-22 已新增 §6.26.10 智慧笔记数据导出导入（Slice P4 note-import）与 §6.26.11 新增规划想法；2026-08-21 已完成存储格式 / 图片 / 题库口径 / C1-C5 去留 / 五维权重裁决，见 §6.26.6 / §6.26.9）
...
- Slice P5（第二切片，原 P4）：行为证据对照——memo / 思考记录 投资关键词抽取（买卖/加仓/减仓/亏损/收益 等）+ 持仓变动 vs 问卷声明一致性打分（确定性规则，不接 LLM）。设计初稿见 §6.26.12；持仓变动对照拆 P5b（依赖持仓导入，§6.26.9 裁决 ⑥），P5 不做空接口。
...
#### 6.26.12 行为证据对照（Slice P5 设计初稿，2026-08-23）

**定位**：季度偏好快照第二切片。把「问卷声明」与「行为文本」做确定性对照，输出逐条证据对照明细 + 聚合一致性得分。与 P1-P4 同口径：确定性规则、不接 LLM。

**证据源与声明源**
- 证据源：`preferences.db` 的 `memos` 表（flomo）+ `thought_records` 表（智慧笔记，§6.26.10 已明确纳入）；仅取季度范围内记录（复用 `quarter_date_range`，与 P3 同边界）。
- 声明源：`questionnaire_results` 截止季度末最近一次作答（与快照基线同口径，`latest_questionnaire_result`）；该季度无作答 → 对照不可用，fail-closed 输出「无问卷基线，无法对照」，不评分。
- 持仓变动对照拆 **P5b**：`holdings` 表尚未实现（依赖持仓导入 slice，`openpyxl` 引入时，§6.26.9 裁决 ⑥）；P5 不定义空接口，快照输出占位「持仓变动对照待持仓导入后启用」。

**声明侧模型**
- 声明维度全集 = 12 道 risk_flag 题（q49-q73）；每道题用户所选选项的**语义类别**即该维度声明。
- 选项语义类别（编码进对照规则表，与题库版本绑定）：
  - `hold`（按计划/继续持有/复盘后决定/分批补仓）：q49-1、q50-0、q52-3、q54-0、q57-0、q61-0、q67-1、q70-1、q73-3；
  - `panic_sell`（立刻卖出/清仓/割肉/离场）：q49-0、q52-0、q54-1、q57-1、q61-1、q67-0、q70-0、q73-1；
  - `leverage`（借钱/加杠杆/满仓）：q49-2、q52-1、q65-2；
  - `emotional`（抱怨/打听/投诉/频繁操作/追热点/弃管）：q49-3、q52-2、q54-3、q57-3、q61-2、q67-3、q73-0；
  - `learn_first`（先学习/评估/小额试水）：q65-0；
  - `knowledge`（认知题，只记录不参与行为对照）：q55、q59；
  - 未归类选项 → 该维度标「未映射」，不参与对照（避免误判）。

**行为侧模型**
- 从证据文本抽取行为事件（动作 + 市场状态 + 情绪三元组），纯关键词匹配（与 P3 同风格），集中定义于新文件 `fund_agent/preferences/behavior.py`：
  - 买入类动作：买入/加仓/申购/定投/补仓/抄底；
  - 卖出类动作：卖出/减仓/赎回/清仓/割肉/止损/止盈/离场；
  - 持有类动作：持有/继续持有/不动/按计划；
  - 下跌类状态：下跌/暴跌/回撤/阴跌/浮亏/亏损/低估/大跌；
  - 上涨类状态：大涨/新高/上涨/涨了/高位/追高；
  - 恐慌类情绪：恐慌/焦虑/害怕/慌/梭哈/割肉。
- 一条证据可命中多个动作/状态；动作缺失或状态缺失 → 该条仅部分可对照（见 R7）。

**对照规则（确定性，规则表集中在 behavior.py）**
- R1 持有纪律：声明含 `hold` 维度（q52/q54/q57/q61/q67/q70/q73 任一用户选 `hold` 类），行为 = 卖出类动作 + 下跌/回撤/浮亏状态 → `inconsistent`（恐慌离场 vs 持有声明）。
- R2 逆向纪律：声明 q70 选 `hold`（低估时持有/补仓），行为 = 买入类动作 + 下跌/低估状态 → `consistent`；行为 = 卖出类动作 + 低估状态 → `inconsistent`。
- R3 追涨：行为 = 买入类动作 + 上涨/新高状态或追高情绪 → 与该用户全部 `hold` 类声明维度 `inconsistent`（追涨 vs 纪律声明）。
- R4 恐慌离场：行为 = 卖出类动作 + 恐慌情绪 → 与 `hold` 类声明 `inconsistent`。
- R5 情绪信号：行为含恐慌/焦虑情绪但无卖出动作 → 记录情绪信号，verdict = `unmapped`（不判违规）。
- R6 学习先行：声明 q65 选 `learn_first`，行为 = 借钱/满仓/全部买入/第一次买 → `inconsistent`。
- R7 无法对照：行为缺状态上下文、或全部声明维度未映射 → `unmapped`，reason 说明原因，不进得分分母。
- 一条证据命中多条规则：任一 `inconsistent` 则整条 `inconsistent`（fail 优先）；否则全 `consistent` 才 `consistent`；否则 `unmapped`；reason 拼接命中规则 id。
- 认知题（q55/q59）永不产生行为对照。

**输出与存储**
- 新增 SQLite 表 `behavior_consistency`（跨季度 SQL 关联，§6.26.4 口径）：`id TEXT PK`（`<snapshot_id>:<序号>`）、`snapshot_id TEXT NOT NULL`（关联 `preference_snapshots.id`）、`evidence_created_at`、`evidence_text`（引用原文，有界截断）、`source`（`memos`/`thought_records`）、`matched_dimensions_json`、`verdict`（`consistent`/`inconsistent`/`unmapped`）、`reason`（命中规则 id + 一句话说明）。
- `preference_snapshots` 加列（兼容既有库 ALTER TABLE）：`consistency_score REAL NULL`（可对照证据一致比例 ×100，无可对照 → NULL）、`consistency_summary_json TEXT NOT NULL DEFAULT '{}'`（consistent/inconsistent/unmapped 计数）。
- markdown 新增「行为证据对照」节：score + 计数 + 明细（时间/来源/原文截断/命中维度/verdict/reason）；无基线或无行为 → 对应占位文案。
- CLI：并入 `preference-snapshot --quarter`，不新增独立命令（MVP 少入口）；json/md 同步落盘。

**合规与边界**
- 不输出调仓/配置建议；免责声明固定文案不变；只做描述性对照，不做因果推断（AGENTS.md 禁止事项同适用）。
- 题库版本绑定：对照规则表按 `baseline-v1` 的 risk_flag 题语义编写；题库 v2 变更时必须重审 R1-R7 映射，版本不匹配 fail-closed（不产出对照，标注「题库版本不匹配」）。
- 隐私：行为原文只本地落盘（work-dir），不进 git；测试 fixture 为构造样例，非私人数据（与 P1-P4 同口径）。

**测试计划（实施时）**
- 单元（behavior.py）：动作/状态/情绪抽取、R1-R7 各规则正反例、认知题不参与、`unmapped` 不进分母、fail 优先。
- 快照集成：真实库（memos + thought_records + 问卷）→ 对照入库 + markdown 渲染；无基线/无行为/无对照边界。
- fixture：构造样例（非私人数据），覆盖 q52 暴跌清仓、q70 低估补仓、q65 借钱满仓等典型对照。
- 最小验证命令：`uv run pytest tests/fund/preferences/ tests/fund/cli/test_cli_preference_snapshot.py -q --tb=short`。

**决策点（实施前需用户确认，不阻塞设计）**
1. 持仓对照拆 P5b（推荐）vs 并入 P5 定义空接口。
2. 对照结果并入快照（推荐）vs 独立 CLI。
3. R3 追涨判定严格度：仅「上涨/新高/追高」命中才算（推荐，避免误伤定投记录）。
