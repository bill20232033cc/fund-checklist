...
_KNOWN_FAILURES: set[tuple[str, int]] = {
    ("512890", 2022),   # e2e fixture 2022 年报 8.3 直接持股 1 条 <5（与 007466-2022 同源 fixture；2026-08-19 归属修正后引用真实 8.3 表）
    ("512890", 2024),   # 年报披露"本基金本报告期末未持有股票"
    ("040046", 2021),   # 前身基金，非联接基金结构
    ("007466", 2021),   # 目标 ETF 2021 年持仓仅 4 条
    ("007466", 2022),   # ETF 联接基金年报直接持股 <5（修正前被错误归属的累计买入/卖出表撑到 ≥5，修正后引用真实 8.3 股票投资明细）
    ("007466", 2023),   # ETF 联接基金年报直接持股 <5（修正前被错误归属的累计买入/卖出表撑到 ≥5，修正后引用真实 8.3 股票投资明细）
