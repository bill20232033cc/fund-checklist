...
- `tests/fund/preferences/` 覆盖 Slice P1 Flomo 导入：`test_flomo_parser.py` 验证解析字段/图片数组/结构错误，`test_flomo_store.py` 验证 SQLite 幂等与错误路径；CLI 端到端在 `tests/fund/cli/test_cli_flomo_import.py`。
- `tests/fund/preferences/test_questionnaire.py` 覆盖 Slice P2 问卷基线：baseline-v1 题库完整性（80 题/板块/难度/risk_flag/权重）与评分（满分/全错/单板块换算/总分=五维和/C1-C5 边界/答案非法），评分单测使用 `fixtures/mini_bank.json` 小题库；CLI 端到端在 `tests/fund/cli/test_cli_preference_questionnaire.py`；`tests/fund/preferences/test_snapshot.py` 覆盖 Slice P3 季度偏好快照（关键词过滤/季度日期范围/四问模板/免责声明/null 基线/落盘 json+md），CLI 端到端在 `tests/fund/cli/test_cli_preference_snapshot.py`。
- `tests/fund/preferences/test_note_parser.py` 覆盖 Slice P4 智慧笔记 note-import（解析字段/类别映射/id 与 ISO8601/声明条数不符/缺分析时间/未知类别/0 条），`test_note_store.py` 覆盖 SQLite 幂等与 `thought_records`/`note_imports` 审计行，CLI 端到端在 `tests/fund/cli/test_cli_note_import.py`。
